...
let url_imprimir = appData.base_url + "Ticket/index";
var cronometro;
...
function get_table1() {
	...
	const tabla_preparando = (data) => {
		...
		if (data.pedidos == null) {
			//console.log("hola" + appData.tipo);
			if (appData.tipo == 3) {
				cuerpo = `<td colspan="3" >No tienes pedidos en espera de envío</td>`;
			} else {
			...
			<a type="button" target="_blank" href="${appData.base_url}Dasboard/pdfticket/${data.pedidos[i].id_pedido}" class="btn btn-outline-secondary btn-sm"><i class="simple-icon-printer"></i></a>
            </td>
			</div>
            </tr>`;
		...
		if (data.res) {
			for (let i = 0; i < data.pedidos.length; i++) {
				...
				const cambio = (data) => {
					...
					if (data.minutos >= tiempo_asignado) {
						$(`#color-time${data.id_pedido}`).addClass("badge-danger");
						$(`#color-time${data.id_pedido}`).removeClass("badge-success");
					} else {
						$(`#color-time${data.id_pedido}`).addClass("badge-success");
                        $(`#color-time${data.id_pedido}`).removeClass("badge-danger");
					}
				};
			}
			setTimeout(function () {
